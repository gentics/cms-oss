...
INFO: The inner content of "JSON":gcn_part_type.html part type can also be resolved, including array indices. For instance, resolving +{{cms.tag.parts.otherpart.whoever.0.whatever}}+ on +{"whatever":{"whoever": "wherever"}, "whoever":[{"whatever": "wherever"}]+ results in +wherever+ being rendered.
...
| "json_path":json_path | Navigate over a "JSON":gcn_part_type.html part type using "JSONPath":https://goessner.net/articles/JsonPath/ | +{{json_path cms.tag.parts.json "$.objarray[0].objfield"}}+ |
...
Helps navigating over a "JSON":gcn_part_type.html part type using "JSONPath":https://goessner.net/articles/JsonPath/ notation input. 
