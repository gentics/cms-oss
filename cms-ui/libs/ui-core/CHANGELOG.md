...
* All Component-Classes have been renamed to include the `Component` suffix.
* All Directive-Classes have been renamed to include the `Directive` suffix.
* All Services/Providers have been renamed to include the `Service`/`Provider` suffix.
* Component Outputs have been renamed from `change` to `valueChange` to adhere the Angular two-way binding.
* `Directive` and `Pipe` selectors are now prefixed with `gtx` like the Components do already.
* The `NotificationService` type `error` has been renamed to the generally used `alert`.
* All components have now proper view/style encapsulation enabled.

### Features

* Added new base classes for Components to inherit from.
