...
Starting from CMP 8.1, Mesh has an ability to generate all the Gentics Image Store image variants, referenced from the content, during the publish run only, disabling the ability to trigger a service denial on random GIS resized image requests in the runtime. This feature can be enabled per Node with the @Publish GIS image variants to Mesh@ checkbox.
