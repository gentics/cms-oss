...
public class FolderSanitizePublishDirTest {
	...
	public void testSegment() throws NodeException {
		FolderPublishDirSanitizeResponse response = assertRequiredPermissions(group, user, () -> {
			return getFolderResource().sanitizePubdir(new FolderPublishDirSanitizeRequest().setPublishDir(publishDir)
	...
	public void testUnknown() throws NodeException {
		operate(t -> {
			getFolderResource().sanitizePubdir(new FolderPublishDirSanitizeRequest().setPublishDir(publishDir).setNodeId(4711));
		});
	}
}
