...
describe('RadioButton', () => {
    ...
    it('prefills a unique "id" if none is passed in',
        componentTest(() => TestComponent, fixture => {
            ...
            const id: Attr | null = nativeInput.attributes.getNamedItem('id');
    ...
    describe('ValueAccessor:', () => {
        ...
        xit('changes a variable bound with ngModel when selected',
            componentTest(() => TestComponent, `
                ...
                ></gtx-radio-button>`,
            (fixture, instance) => {
                (instance as any).boundProperty = undefined;
                fixture.detectChanges();
                tick();
                expect(instance.checkState).toBe(false);
                expect(instance.boundProperty).toBeUndefined();
...
describe('RadioGroup', () => {
    ...
    it('sets a property bound with ngModel to null when no RadioButton children are checked (outbound)',
        ...
        (fixture, instance) => {
            ...
            tick();
            fixture.detectChanges();
            expect(instance.boundProperty).toBeNull();
            expect(nativeInputs[0].checked).toBe(false);
            expect(nativeInputs[1].checked).toBe(false);
...
class TestComponent {
    ...
    boundProperty = 'boundValue';
    checkState = false;
    testForm: UntypedFormGroup = new UntypedFormGroup({
        testControl: new UntypedFormControl('controlValue'),
    });
    ...
    @ViewChild(RadioButtonComponent, { static: true })
