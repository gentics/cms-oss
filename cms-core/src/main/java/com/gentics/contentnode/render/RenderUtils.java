			...
			if (masterNode.doPublishContentmap()) {
				...
				if (cr != null && cr.getCrType() == Type.mesh && !StringUtils.isEmpty(meshPreviewUrl)) {
					...
					try (
						...
						mp.handleRenderedEntries(true, masterNode.getId(), page.getId(), page.getTType(), () -> meshNode.getFields(), mp.render(mp.getEntries(Page.TYPE_PAGE), null, language, false), null, null, null, null);
