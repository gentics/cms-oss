    ...
    describe('FolderInstancePermissionsImpl', () => {
        ...
        describe('hasPermissionOrRolePrivilege()', () => {
            ...
            function assertHasPermOrPrivWorksWithoutRoles(itemType: FolderItemType): void {
                ...
                expect(folderPermissions.hasPermissionOrRolePrivilege('page', GcmsRolePrivilege.CREATE_ITEMS)).toBe(false);
            }
            ...
            it('checks the permissions first and maps the privilege correctly to a permission', () => {
                ...
                assertHasPermOrPrivWorks('page', GcmsRolePrivilege.READ_ITEMS, true);
            });

            it('does not check permissions for translatepages', () => {
                const result = folderPermissions.hasPermissionOrRolePrivilege('page', GcmsRolePrivilege.TRANSLATE_PAGES);
            ...
            it('throws an error when checking a non applicable privilege', () => {
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('file', GcmsRolePrivilege.PUBLISH_PAGES)).toThrow();
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('file', GcmsRolePrivilege.TRANSLATE_PAGES)).toThrow();
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('image', GcmsRolePrivilege.PUBLISH_PAGES)).toThrow();
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('image', GcmsRolePrivilege.TRANSLATE_PAGES)).toThrow();
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('folder', GcmsRolePrivilege.PUBLISH_PAGES)).toThrow();
                expect(() => folderPermissions.hasPermissionOrRolePrivilege('folder', GcmsRolePrivilege.TRANSLATE_PAGES)).toThrow();
            });
            ...
            describe('file', () => {
                ...
                it('checks privileges if the permission is set to false and returns true for a granted privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.createitems).toBe(false);
                    assertHasPermOrPrivWorks('file', GcmsRolePrivilege.CREATE_ITEMS, true);
                });
                ...
                it('checks privileges if the permission is set to false and returns false for a denied privilege', () => {
                    ...
                    assertHasPermOrPrivWorks('file', GcmsRolePrivilege.DELETE_ITEMS, false);
                });
            ...
            describe('image', () => {
                ...
                it('checks privileges if the permission is set to false and returns true for a granted privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.createitems).toBe(false);
                    assertHasPermOrPrivWorks('image', GcmsRolePrivilege.CREATE_ITEMS, true);
                });

                it('checks privileges if the permission is set to false and returns false for a denied privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.deleteitems).toBe(false);
                    assertHasPermOrPrivWorks('image', GcmsRolePrivilege.DELETE_ITEMS, false);
                });
            ...
            describe('folder', () => {
                ...
                it('checks permissions only', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.createitems).toBe(false);
                    assertHasPermOrPrivWorks('folder', GcmsRolePrivilege.CREATE_ITEMS, false);
                });
            ...
            describe('page', () => {
                ...
                it('without language: checks privileges if the permission is set to false and returns true for a granted privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.createitems).toBe(false);
                    assertHasPermOrPrivWorks('page', GcmsRolePrivilege.CREATE_ITEMS, true);
                });

                it('without language: checks privileges if the permission is set to false and returns false for a denied privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.deleteitems).toBe(false);
                    assertHasPermOrPrivWorks('page', GcmsRolePrivilege.DELETE_ITEMS, false);
                });
                ...
                it('with language: checks language privileges if permissions and type privileges are false and returns true for a granted privilege', () => {
                    ...
                    assertHasPermOrPrivWorksWithLanguage(GcmsRolePrivilege.PUBLISH_PAGES, 'en', true);
                });
                ...
                it('with language: checks language privileges if permissions and type privileges are false and returns false for a denied privilege', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.publishpages).toBe(false);
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.rolePermissions.page.publishpages).toBe(false);
                    assertHasPermOrPrivWorksWithLanguage(GcmsRolePrivilege.PUBLISH_PAGES, 'de', false);
                });

                it('with language: returns false if the language does not exist', () => {
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.permissions.publishpages).toBe(false);
                    expect(MOCK_FOLDER_INSTANCE_PERMISSIONS_MAP_COLLECTION.rolePermissions.page.publishpages).toBe(false);
                    assertHasPermOrPrivWorksWithLanguage(GcmsRolePrivilege.PUBLISH_PAGES, 'it', false);
                });
                ...
                it('with language: works if no language specific privileges are set', () => {
                    ...
                    expect(folderPermissions.hasPermissionOrRolePrivilege('page', GcmsRolePrivilege.PUBLISH_PAGES)).toBe(false);
                });
