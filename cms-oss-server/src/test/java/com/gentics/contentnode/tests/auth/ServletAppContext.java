...
public class ServletAppContext extends ExternalResource {
	...
	public ServletAppContext baseResource(Path path) {
		handler.setBaseResourceAsPath(path);
		return this;
	}
	...
	@SuppressWarnings("unchecked")
	public ServletAppContext filter(String path, Class<? extends Filter> filterClass, Pair<String, String>... params) {
		FilterHolder filterHolder = new FilterHolder(filterClass);
		for (Pair<String, String> param : params) {
			filterHolder.setInitParameter(param.getLeft(), param.getRight());
		}
		handler.addFilter(filterHolder, path, EnumSet.of(DispatcherType.REQUEST));
		return this;
	}
