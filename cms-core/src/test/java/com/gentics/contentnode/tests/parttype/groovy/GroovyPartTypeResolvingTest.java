...
import static com.gentics.contentnode.tests.parttype.groovy.AbstractGroovyTest.GROOVY_PART_NAME;
import static com.gentics.contentnode.tests.parttype.groovy.AbstractGroovyTest.GROOVY_TAGNAME;
import static com.gentics.contentnode.tests.parttype.groovy.AbstractGroovyTest.HBS_PART_NAME;
...
public class GroovyPartTypeResolvingTest {
	@ClassRule
	...
	public void setup() throws NodeException {
		...
		construct = update(construct, c -> {
			getPartType(HandlebarsPartType.class, c, HBS_PART_NAME).setText("{{ cms.tag.parts.%s.execute }}".formatted(GROOVY_PART_NAME));
			getPartType(GroovyPartType.class, c, GROOVY_PART_NAME).setText("return cms.tag.parts.otherpart.keySet().join(\", \")");
		}).build();
		...
		template = create(Template.class, t -> {
			...
			t.setMlId(1);
			t.setName("Template");
			t.setSource(String.format("<node %s>", GROOVY_TAGNAME));
	...
	/**
	 * Test rendering the page (which renders the tag)
	 * @throws NodeException
	 */
	@Test
	public void testRenderPage() throws NodeException {
	...
	/**
	 * Test rendering the handlebars part directly
	 * @throws NodeException
	 */
	@Test
	public void testRenderHandlebarsPart() throws NodeException {
		try (Trx trx = new Trx(); RenderTypeTrx rTrx = RenderTypeTrx.publish()) {
			assertThat(page.render("<node %s.parts.%s>".formatted(GROOVY_TAGNAME, HBS_PART_NAME),
	...
	/**
	 * Test executing the groovy part directly
	 * @throws NodeException
	 */
	@Test
	public void testExecuteGroovyPart() throws NodeException {
		try (Trx trx = new Trx(); RenderTypeTrx rTrx = RenderTypeTrx.publish()) {
			assertThat(page.render("<node %s.parts.%s.execute>".formatted(GROOVY_TAGNAME, GROOVY_PART_NAME),
					trx.getTransaction().getRenderResult(), null, null, null, null)).as("Rendered page")
					.isEqualTo(expectedResolvableKeys);
			trx.success();
		}
