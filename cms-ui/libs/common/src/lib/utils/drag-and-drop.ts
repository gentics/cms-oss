export function getDataTransfer(event: Event): DataTransfer | null {
    if (event == null) {
        return null;
    }
    // if jQuery wrapped the event which contains the dropped file, unwrap it
    const ev: DragEvent = (event as DragEvent).dataTransfer
        ? event
...
export function getEventTarget(event: Event | JQuery.TriggeredEvent): HTMLElement | null {
    if (event == null) {
        return null;
    }
    // if jQuery wrapped the event, unwrap it

    return ((event as JQuery.TriggeredEvent).originalEvent ? (event as JQuery.TriggeredEvent).originalEvent : event)!.target;
}
...
export function getTransferMimeTypes(transfer: DataTransfer): string[] {
    if (!transfer) {
        ...
        return new Array((transfer as any).mozItemCount).fill(FALLBACK_MIME_TYPE);
