 ...
 * @author norbert
 * @date 04.07.2006
 * @version $Id: CNDatasourceExtendedComparisonFunction.java,v 1.1 2010-02-03 09:32:49 norbert Exp $
 */
...
public class CNDatasourceExtendedComparisonFunction extends AbstractBinaryCNDatasourceFunction {
	...
	private void variableWithStaticContainsOneOfComparison(ExpressionQueryRequest request,
		...
		try {
			...
			CNDatasourceFilter filter = (CNDatasourceFilter) request.getFilter();
	...
	public static class ContainsOneofFilterPartGenerator implements FilterPartGenerator {
		...
		public FilterPart getFilterPart(ExpressionQueryRequest request) throws ExpressionParserException {
			...
			CNDatasourceFilter filter = (CNDatasourceFilter) request.getFilter();
			CNDatasourceFilter newFilter = filter.createSubFilter();
