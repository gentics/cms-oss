...
/**
 * Sorting and filtering tests for {@link UserResource#list(FilterParameterBean, SortParameterBean, PagingParameterBean, PermsParameterBean)}
 */
public class UserResourceListTest extends AbstractListSortAndFilterTest<User> {
	...
	protected AbstractListResponse<User> getResult(SortParameterBean sort, FilterParameterBean filter, PagingParameterBean paging) throws NodeException {
		return new UserResourceImpl().list(filter, sort, paging, new PermsParameterBean(), new EmbedParameterBean());
	}
}
