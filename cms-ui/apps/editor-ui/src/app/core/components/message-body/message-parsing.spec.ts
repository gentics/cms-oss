...
describe('Message parsing', () => {
    ...
    describe('parseMessage()', () => {
        ...
        it('parses "{{user}} wants to publish the page with ID {{id}} at {{date}}" messages (en)', () => {
            let text = 'Without Publish wants to publish the page with ID 343 at 31.08.2019 09:08:38.';
            let result = parseMessage(text);
            expect(result.links).toEqual([
                {
                    textBefore: 'Without Publish wants to publish the page with ID ',
                    type: 'page',
                    id: 343,
                    name: null,
                    nodeName: '',
            ...
            expect(result.textAfterLinks).toBe(' at 31.08.2019 09:08:38.');
        });

        it('parses "{{user}} wants to publish the page with ID {{id}} at {{date}}" messages (de)', () => {
            let text = 'Without Publish möchte die Seite mit der ID 343 am 31.08.2019 09:08:38 veröffentlichen.';
            let result = parseMessage(text);
            expect(result.links).toEqual([
                {
                    textBefore: 'Without Publish möchte die Seite mit der ID ',
                    type: 'page',
                    id: 343,
                    name: null,
                    nodeName: '',
            ...
            expect(result.textAfterLinks).toBe(' am 31.08.2019 09:08:38 veröffentlichen.');
        });
    ...
    describe('placeholderStringToRegExp', () => {
        ...
        it('works for strings with placeholder "id"', () => {
            let result = placeholderStringToRegExp('Page id is {{id}}!');
            expect(result.placeholders).toEqual(['id']);
            expect(result.regExp).toEqual(/^(Page id is )([\s\S]*?)(!)$/);
        });
