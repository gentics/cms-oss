...
INFO: The inner content of "JSON":gcn_part_types.html part type can also be resolved, including array indices. For instance, resolving +{{cms.tag.parts.otherpart.whoever.0.whatever}}+ on +{"whatever":{"whoever": "wherever"}, "whoever":[{"whatever": "wherever"}]+ results in +wherever+ being rendered.

The Handlebars Part Type does not support "shortcuts" when resolving. So for example to resolve a specific part in the current tag,
...
| "gtx_script":#gtx_script | Execute a script from a devtool package and render the output | +{{gtx_script "demopackage.scriptname" page=cms.page}}+ |
| "gtx_json_path":#gtx_json_path | Navigate over a "JSON":gcn_part_type.html part type using "JSONPath":https://goessner.net/articles/JsonPath/ | +{{gtx_json_path cms.tag.parts.json "$.objarray[0].objfield"}}+ |
...
The channel scope influences the @resolving@ (loading) of objects, that take part in multichannelling. It is important to note, that the channel scope does *not* change
the objects, which are *directly* referenced via @cms@ (like @cms.node@, @cms.page@, ...), because those objects have already been loaded.

Objects, which are further resolved, however, *will* be influenced by the channel scope. The following example should make this clear:

<html>
{{#gtx_channel [channel]}}
  <!-- Examples, which are independent of the channel scope -->
  {{cms.node}} <!-- This is always the current node -->
  {{cms.page}} <!-- This is always the current page -->
  {{cms.folder}} <!-- This is always the current folder -->

  <!-- Examples, which depend on the channel scope -->
  {{cms.page.folder}} <!-- This is the current page's folder in the scope of the channel -->
  {{cms.page.node}} <!-- This is the current page's node in the scope of the channel -->
  {{cms.folder.node}} <!-- This is the current folder's node in the scope of the channel -->
...
<shell>
{{{gtx_form form [type=value] [previewRoot=value] [showResults=value] }}}
</shell>
...
This helper executes a groovy script from a devtool package and renders the output.

General usage is:

<shell>
{{gtx_script [scriptname] [param1=value1 param2=value2 ...]}}
</shell>

With the parameters

|_. Name |_. Description |_. Default |
| +scriptname+ | Fully qualified name of the script to be executed | _mandatory_ |

All other parameters will be injected into the script context. See "Groovy Scripts in Devtool Packages":groovy.html#groovy-scripts-in-devtool-packages for details.

h5. gtx_json_path

Helps navigating over a "JSON":gcn_part_types.html part type using "JSONPath":https://goessner.net/articles/JsonPath/ notation input. 

General usage is:

<shell>
{{gtx_json_path [object] [JSONPath-string]}}
</shell>

Where the parameters are

|_. Name |_. Description |_. Default |
| +object+ | JSON part type object to resolve from | _mandatory_ |
| +JSONPath-string+ | JSONPath notated string | _mandatory_ |

An example of rendering a JSON part type "otherpart" of a current tag, having content
...
<shell>
"wherever, wherever, who"
</shell>

Please refer to the "implementing library documentation":https://github.com/json-path/JsonPath/blob/master/README.md for the whole possibilities of JSONPath.
...
WARNING: The usage of custom helpers from devtool packages will *not* be tracked by the internal dependency management. If any helper in a
...
WARNING: The usage of partials from devtool packages will *not* be tracked by the internal dependency management. If any partial in a
devtool package is modified, republishing of the affected objects needs to be done manually.
