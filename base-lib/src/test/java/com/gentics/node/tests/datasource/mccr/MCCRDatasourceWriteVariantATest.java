...
public class MCCRDatasourceWriteVariantATest extends AbstractMCCRDatasourceWriteTest {

	public static final boolean USE_DIFFERENT_CONTENTID = true;
	 ...
	 */
	public MCCRDatasourceWriteVariantATest(TestDatabase testDatabase, String attribute, boolean useDiffetentContentId, boolean useDifferentAttrValue,
		...
		for (TestDatabase testDB : variations.values()) {
			for (String attribute : EXPECTEDDATA_MASTER.keySet()) {
				if (!isExcluded(testDB, attribute)) {
					...
					boolean useDifferentAttrValue = false;
