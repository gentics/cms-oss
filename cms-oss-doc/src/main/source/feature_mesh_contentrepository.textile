...
These tagmap entries will validate its contents against "JavaScript Object Notation":https://www.json.org/json-en.html . Additionally, one may limit the allowed content with JSON schemas. As the simplest example of allowance limits, the following JSON schemas depict the person data, and a simple array of strings:
