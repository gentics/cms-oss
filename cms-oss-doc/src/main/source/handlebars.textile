...
Hence, if you need to, say, render a copyright of a referenced image, set withing an image's object property, the Velocity's definition +{{cms.tag.parts.img.object.copyright}}+ turns into a longer variant for the Handlebars: 
+{{cms.tag.parts.img.target.object.copyright.parts.text.text}}+, where +target+ image dereferencing, +parts+ property listing, and +text+ part type now have to be explicitly mentioned, for the sake of clarity.
