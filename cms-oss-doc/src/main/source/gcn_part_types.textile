...
| 42        | Form                    | The CMS Form part lets the user select a form from the CMS. See "Gentics CMS Forms":feature_forms.html for details. |
...
| 44        | JSON                    | The JSON part type offers storing, validation and navigation/resolving over structured data in the "JavaScript Object Notation":https://www.json.org/json-en.html content. | 
| 45        | Groovy                  | The Groovy part type allows to execute Groovy Scripts for preparation of data to be rendered. See "Implementation of Groovy Scripts":groovy.html for details. |

NOTE: Inline editable parts - which are transformed to editables in Aloha Editor - should only use the part type @HTML@.

h3. JSON Part Type

This part type is designed to work with content of the "JavaScript Object Notation":https://www.json.org/json-en.html . The tag content of this type is validated against JSON syntax.
...
For looking up into the full possibilities of JSON schemas filtering one may inspect the link:https://json-schema.org/[official JSON Schema guide].
