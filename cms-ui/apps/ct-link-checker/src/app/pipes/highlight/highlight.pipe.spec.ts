...
describe('HighlightPipe:', () => {
     ...
     * test it.
     */
    function transformAndUnwrap(testStringParam: string, term: string): string {
        return highlightPipe.transform(testStringParam, term) as string;
    }
    ...
    it('should return original string with unexpected term', () => {
        expect(transformAndUnwrap(testString, 1 as any)).toBe(testString);
        expect(transformAndUnwrap(testString, undefined as any)).toBe(testString);
        expect(transformAndUnwrap(testString, null as any)).toBe(testString);
        expect(transformAndUnwrap(testString, false as any)).toBe(testString);
        expect(transformAndUnwrap(testString, {} as any)).toBe(testString);
        expect(transformAndUnwrap(testString, [] as any)).toBe(testString);
    });
