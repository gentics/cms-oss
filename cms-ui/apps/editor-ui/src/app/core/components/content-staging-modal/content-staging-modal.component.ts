...
import { ApplicationStateService, SetActiveContentPackageAction, SetUIModeAction, ChangeListSelectionAction } from '@editor-ui/app/state';
...
export class ContentStagingModal extends BaseModal<void> implements OnInit {
    ...
    updatePackageInState(): void {
        if (this.activePackage) {
            this.state.dispatch(new ChangeListSelectionAction(null, 'clear'));
