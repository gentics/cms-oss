...
/**
 * Sorting and filtering tests for {@link GroupResource#subgroups(String, FilterParameterBean, SortParameterBean, PagingParameterBean, PermsParameterBean)}
 */
public class GroupResourceSubgroupsTest extends AbstractListSortAndFilterTest<Group> {
	...
	protected AbstractListResponse<Group> getResult(SortParameterBean sort, FilterParameterBean filter, PagingParameterBean paging) throws NodeException {
		return new GroupResourceImpl().subgroups(Integer.toString(ContentNodeTestDataUtils.NODE_GROUP_ID), filter, sort, paging, new PermsParameterBean());
	}
}
